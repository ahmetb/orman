...
/**
 * {@link Criterion} and logical statement generator
 * façade for objective queries. Extends {@link org.orman.sql.C}
 * such that it can handle all {@link Criterion} producing methods
 * and those which work with actual {@link Entity}es and {@link Field}s.
 * 
 * @author alp
 *
 */
public class C extends org.orman.sql.C {
	public static Criterion eq(Class<?> entityClass, String fieldName, Object val) {
		return C.eq(F.formatField(entityClass, fieldName), val);
	}
	
	public static Criterion notEq(Class<?> entityClass, String fieldName, Object val) {
		return C.notEq(F.formatField(entityClass, fieldName), val);
	}
